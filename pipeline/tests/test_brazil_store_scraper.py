...
def brazil_store_products():
    """Load Brazil Store mock product data from fixtures."""
    data = load_mock_response("brazil_store_response.json")
    return data["data"]["products"]


@pytest.fixture
def mock_app_success(brazil_store_products):
    """FirecrawlApp mock returning valid Brazil Store products."""
    app = MagicMock()
    app.extract = MagicMock(return_value={"data": {"products": brazil_store_products}})
    return app
...
class TestExtractProductsSchema:
    ...
    def test_price_currency_validation(self, brazil_store_products):
        """All prices are positive floats (BRL values)."""
        for product in brazil_store_products:
    ...
    def test_affiliate_url_format(self, brazil_store_products):
        """product_url contains the Brazil Store domain."""
        for product in brazil_store_products:
    ...
    def test_image_url_validity(self, brazil_store_products):
    ...
    def test_brand_not_empty(self, brazil_store_products):
...
class TestRetryBehavior:
    ...
    def test_timeout_retry_behavior(self):
        ...
        app = MagicMock()
        success_response = {"data": {"products": [
            {"name": "Test", "price_brl": 500.0, "in_stock": True,
             "image_url": "https://img.com/1.jpg",
             "product_url": "https://brazilpickleballstore.com.br/1",
             "brand": "TestBrand", "specs": {}}
        ]}}
    ...
    def test_rate_limit_backoff(self):
        ...
        app = MagicMock()
        app.extract = MagicMock(side_effect=RateLimitError("429 rate limit"))
    ...
    async def test_parse_error_handling(self):
        ...
        app = MagicMock()
        # extract succeeds but returns no products (parse-level issue)
...
class TestSaveProductsToDb:
    ...
    async def test_happy_path_saves_all_products(self, brazil_store_products, scraper_db_connection):
        ...
        assert scraper_db_connection.execute.call_count == len(brazil_store_products)
